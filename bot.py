...
def DoesnthaveAcc(ctx):
    with open(path.join(cwd, "server\\user_stats.json"),"r") as users:
        u = ctx.author.id
        if str(u) not in json.load(users):
            embed=discord.Embed(title="You do not have an account scammer! D:",description="Please make an account using the `verify` command!",colour=ctx.author.roles[-1].color)
...
async def on_ready():
    ...
    for guild in bot.guilds:
        ...
        for r in guild.roles:
            if r.name == "MOOD_Verified":
                has = True
                break
        if not has:
            await guild.create_role(name="MOOD_Verified",colour=discord.Colour.dark_gold())
...
async def verify(ctx):
    ...
    embed.set_footer(text=version)
    await user.send(embed=embed)
    await ctx.author.add_roles(discord.utils.get(ctx.guild.roles, name="MOOD_Verified"))

@bot.command(pass_context=True,aliases=["stat","s"])
@commands.guild_only()
#@commands.cooldown(3,60,commands.BucketType.user)
async def status(ctx, member: discord.Member=None):
    DHA = DoesnthaveAcc(ctx)
    if type(DHA) != bool:
        await ctx.send(embed=DHA)
    if member == None:
        member = ctx.author
    user = member
    ID = user.id
